  ...
  public PaymentRestConnectorImpl(
    ...
    public TransactionBarCodeEnrichedResponse createExtendedTransaction(TransactionBarCodeCreationRequest trxBarCodeCreationRequest, String userId) {
      try{
          ...
          if (e.status() == 403){
              log.error("[CREATE_EXTENDED_TRANSACTION] Voucher generation for the user {} received an error as the initiative {} has ended", userId, trxBarCodeCreationRequest.getInitiativeId());
